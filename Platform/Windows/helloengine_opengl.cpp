...
bool InitializeOpenGL(HWND hwnd, int screenWidth, int screenHeight, float screenDepth, float screenNear, bool vsync)
{
	...
	if(result != 1)
	{
		MessageBoxW(hwnd, L"InitializeOpenGL Error", L"InitializeOpenGL Error", MB_OK);
		return false;
	}
    ...
    int nCmdShow)
{
	...
	{
		MessageBoxW(hWnd, L"InitializeExtensions Error", L"InitializeExtensions Error", MB_OK);
	}
    ...
    if(!InitializeOpenGL(hWnd, 960, 540, SCREEN_DEPTH, SCREEN_NEAR, true))
	{
		MessageBoxW(hWnd, L"InitializeOpenGL Error", L"InitializeOpenGL Error", MB_OK);
	}
